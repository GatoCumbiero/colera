""" Compare polaron redfield with normal Redfield."""
...
def calculate_multiple_dynamics():
    N = 10
    ...
    dynamics = calculate_full_dynamics(parameters)
    dynamics_redfield = calculate_tdep_redfield_dynamics_polaron(parameters)
    dynamics_tdep_redfield = calculate_tdep_redfield_dynamics(parameters)
    dynamics_tdep_redfield_polaron = calculate_tdep_redfield_dynamics_polaron(parameters)

    return dynamics, dynamics_redfield, dynamics_tdep_redfield, dynamics_tdep_redfield_polaron, parameters

dynamics, dynamics_redfield, dynamics_tdep_redfield, dynamics_tdep_redfield_polaron, parameters = calculate_multiple_dynamics()
...
params_text.append(r"$\Delta_q +\chi(2Re(\alpha_g\alpha_g^{*})+1)$:"+f"{qubit_targetz/(2*np.pi*1e6):.1f}")

...
def heatmap_eff_coupling(eff_coupling, save_path="./eff_coupling_results"):
    ...
    cavity_diss_rate = 2*np.pi*2.3*10**6  # Fixed cavity dissipation rate
    norm = eff_coupling
    final_time__mus = 2
    time_steps = 1000
    ...
    filename = f"coupling_sweep_g_{eff_coupling/(2*np.pi*1e6):.1f}MHz_{timestamp}.npz"
    filepath = os.path.join(save_path, filename)
    
    np.savez(filepath, **results)
    
    # Also save as pickle for easier loading of QuTiP objects if needed
    pickle_filename = f"coupling_sweep_g_{eff_coupling/(2*np.pi*1e6):.1f}MHz_{timestamp}.pkl"
...
def main():
    # Test with different effective coupling strengths
    eff_couplings = [2*np.pi*(2.5)*10**6]  # Add more couplings if desired
    
    for coupling in eff_couplings:
        print(f"Calculating heatmap for g = {coupling/(2*np.pi*1e6):.1f} MHz", flush=True)
        results = heatmap_eff_coupling(coupling)
        
        # You can save results if needed
        # np.savez(f'heatmap_coupling_{coupling/(2*np.pi*1e6):.1f}MHz.npz', **results)

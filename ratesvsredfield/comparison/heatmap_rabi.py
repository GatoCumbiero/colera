...
def heatmap_rabi_freq(rabi_freq, save_path="./rabi_freq_results"):
    ...
    N = 10
    cavity_diss_rate = 2*np.pi*4.3*10**6  # Fixed cavity dissipation rate
    ...
    for i in range(dinput):
        ...
        for j in range(dtarget):
            ...
            print(f"Progress: {current_simulation}/{total_simulations} "
                  ...
                  f"target frequency: {qubit_targetz_mesh[i, j]/(2*np.pi*1e6):.1f} MHz)")
    ...
    filename = f"rabi_sweep_omega_{rabi_freq/(2*np.pi*1e6):.1f}MHz_{timestamp}.npz"
    filepath = os.path.join(save_path, filename)
    
    np.savez(filepath, **results)
    
    # Also save as pickle for easier loading of QuTiP objects if needed
    pickle_filename = f"rabi_sweep_omega_{rabi_freq/(2*np.pi*1e6):.1f}MHz_{timestamp}.pkl"
...
def main():
    ...
    rabi_freq = [2*np.pi*27*10**6]  # Add more rates if desired
    
    for rate in rabi_freq:
        print(f"Calculating heatmap for rabi_freq = {rate/(2*np.pi*1e6):.1f} MHz", flush=True)
        results = heatmap_rabi_freq(rate)
